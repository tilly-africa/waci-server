...
import {
  clearCredentialrefCookie,
  clearWebSocketCookie,
  setCredentialrefCookie,
} from "@server/cookies";
...
} from "@server/waciJose";

import { VC, signVC, VP, signVP, verifyVP } from "@bloomprotocol/vc";
import {
  keyUtils,
  EcdsaSecp256k1VerificationKey2019,
} from "@bloomprotocol/ecdsa-secp256k1-verification-key-2019";
...
export const applyCredentialrefRoutes = (app: FastifyInstance): void => {
  ...
  const getPublicJwkForKey = async (
    did: string,
    keyId: string,
    didDoc: any
  ) => {
    const l = keyUtils.publicKeyJWKFrom.publicKeyHex(
      didDoc.publicKey[0]["publicKeyHex"],
      keyId
    );
    return l;
  };
  ...
  const getResponseTokenKey: JWTVerifyGetKey = async (header, token) => {
    ...
    if (didDocument.id.startsWith("did:elem")) {
      const parsedjwk = await parseJwk(
        await getPublicJwkForKey(payload.iss, header.kid, didDocument),
        header.alg
      );
      return parsedjwk;
    } else {
          ...
          const sig = new EcdsaSecp256k1Signature2019({
            key: EcdsaSecp256k1VerificationKey2019.from({
              ...
              publicKeyBase58: base58.encode(
                Buffer.from(didDocument.publicKey[0]["publicKeyHex"], "hex")
              ),
  ...
  app.post<shared.api.waci.offer.submit.RouteInterface>(
    ...
    async (req, reply) => {
      ...
      try {
        ...
        await useToken(
          req.body.responseToken,
          new Date(result.challenge.payload.exp * 1000)
        );
      } catch (e) {
        console.log(e);
      ...
      const { id: credentialrefId } = await credentialrefsRepo.save(
        Credentialref
      );
      let Credential;

      try {
        Credential = await Credentials.getRepo().findOneOrFail({
  ...
  app.get<shared.api.waci.request.challengeToken.RouteInterface>(
    ...
    async (req, reply) => {
      const challengeToken = await new SignRequestChallengeJWT({
        ...
        callbackUrl:
          app.env.waciHost + `${shared.api.waci.request.submit.path}`,
        version: "1",
      })
        .setProtectedHeader({ alg: "ES256K", kid: app.key.keyPair.id })
        .setSubject(req.query.token)
  ...
  app.post<shared.api.waci.request.submit.RouteInterface>(
    ...
    async (req, reply) => {
      ...
      const presentation: Object = result.response.payload[
      ...
      try {
        ...
        const credentialref = credential.credentialSubject["id"].replace(
          "urn:uuid:",
          ""
        );
        const Credentialref = await Credentialrefs.getRepo().findOneOrFail({
          where: { credentialref },
        });
  ...
  app.post<shared.api.waci.request.authenticate.RouteInterface>(
    ...
    async (req, reply) => {
      ...
      let result;
      try {
        result = await jwtVerify(req.body.token, app.key.keyLike, {
          issuer: app.key.keyPair.controller,
        });
        ...
        return clearWebSocketCookie(reply)
          .status(400)
          .send({
            success: false,
      ...
      return setCredentialrefCookie(reply, result.payload.sub)
        .status(200)
        .send({
          success: true,
        });
  ...
  app.post<shared.api.waci.available.RouteInterface>(
    ...
    async (req, reply) => {
      const count = await Credentialrefs.getRepo().count({
